...
#    print("{lat: ", data['places'][manager.IndexToNode(index)]['Lat'], ", lng: ",
#          data['places'][manager.IndexToNode(index)]['Long'], "}, //",
#          data['places'][manager.IndexToNode(index)]['Place'])
...
def print_solution(manager, routing, assignment, data, printactivities):
    ...
    while not routing.IsEnd(index):
        if printactivities:
            result.append(
                (float(data['places'][manager.IndexToNode(index)]['Long']),
                 float(data['places'][manager.IndexToNode(index)]['Lat'])))
            plan_output += '{} ({}) -> '.format(data['places'][manager.IndexToNode(index)]['Place'],
                                                data['places'][manager.IndexToNode(index)]['Activity'])
        else:
            plan_output += '{} -> '.format(data['places'][manager.IndexToNode(index)]['Place'])
        previous_index = index
        index = assignment.Value(routing.NextVar(index))
...
def create_data_model(coords_raw_data):
    ...
    data = {}
    data['places'] = [{
        "Place": str(long) + "," + str(lat),
        "Activity": "t",
        "Lat": lat,
        "Long": long
    } for long, lat in coords_raw_data]  # places
...
def main(coords_raw_data):
    ...
    data_model = create_data_model(coords_raw_data)
    ...
    if assignment:
        ...
        test = print_solution(manager, routing, assignment, data_model, True)
        print(str(test))
        return test
    else:
